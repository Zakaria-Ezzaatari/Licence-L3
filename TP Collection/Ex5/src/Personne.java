...
public class Personne implements Comparable<Personne>
{
	...
	public int compareTo(Personne per) 
	{
        ...
        if (this.dateNaissance.compareTo(per.dateNaissance) < 0)
        {
            res = -1;
        }
        else if (this.dateNaissance.compareTo(per.dateNaissance) > 0)
        {
            res = 1;
        }
        else 
        {
            res = this.nom.compareTo(per.nom);
            if (res == 0)
            {
                res = this.prenom.compareTo(per.prenom);
            }
